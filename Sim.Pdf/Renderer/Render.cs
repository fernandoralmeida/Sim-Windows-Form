...
namespace Sim.Pdf.Renderer
{
    public class Render
    {
        ...
        /// </summary>
        public void Page(XGraphics gfx)
        {
            XRect rect;
            XPen pen;
            double x = 50, y = 100;
            ...
            //gfx.DrawImage(XPdfForm.FromFile(@"D:\Documentos\Visual Studio 2015\Projects\PDFsharp\samples\PDFs\Teste.pdf"), rcImage);
            gfx.DrawImage(XImage.FromFile(@"D:\Imagens\images0VC173U7.jpg"), rcImage);
            gfx.Restore(state);
        }
        ...
        public void Imagen(XGraphics gfx)
        {
            XGraphicsState state = gfx.Save();
            XRect rcImage = new XRect(PageSizeConverter.ToSize(PageSize.A4));
            gfx.DrawRectangle(XBrushes.Snow, rcImage);
            //gfx.DrawImage(XPdfForm.FromFile(@"Cache\leis.jau.sp.gov.brleis_complementares2005265.pdf"), rcImage);
            
            gfx.DrawImage(XImage.FromFile(string.Format(@"Cache\{0}\{1}.png", folder, indice + 1)), rcImage);
            gfx.Restore(state);
        } 
